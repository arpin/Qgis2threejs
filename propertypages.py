    ...
    def properties(self):
        ...
        for w in self.propertyWidgets:
            ...
            if isinstance(w, QComboBox):
                ...
                logMessage(
                    "[propertypages.py] Not recognized widget type: " +
                    unicode(
                        type(w)))
...
class WorldPropertyPage(PropertyPage, Ui_WorldPropertiesWidget):
    ...
    def __init__(self, dialog, parent=None):
        ...
        self.registerPropertyWidgets([self.lineEdit_BaseSize,
    ...
    def setup(self, properties=None):
        ...
        self.lineEdit_MapCanvasExtent.setText(
            "%.4f, %.4f - %.4f, %.4f" %
            (extent.xMinimum(),
             extent.yMinimum(),
             extent.xMaximum(),
             extent.yMaximum()))
        self.lineEdit_MapCanvasSize.setText(
            "{0} x {1}".format(
                outsize.width(),
                outsize.height()))
    ...
    def colorButtonClicked(self):
        color = QColorDialog.getColor(
            QColor(
                self.lineEdit_Color.text().replace(
                    "0x", "#")))
        if color.isValid():
...
class ControlsPropertyPage(PropertyPage, Ui_ControlsPropertiesWidget):
    ...
    def __init__(self, dialog, parent=None):
        ...
        self.controlsDir = os.path.join(
            tools.pluginDir(), "js", "threejs", "controls")
...
class DEMPropertyPage(PropertyPage, Ui_DEMPropertiesWidget):
    ...
    def __init__(self, dialog, parent=None):
        ...
        widgets += [self.checkBox_Surroundings,
                    self.spinBox_Size, self.spinBox_Roughening]
        widgets += [self.radioButton_Advanced,
                    self.spinBox_Height,
                    self.lineEdit_centerX,
                    self.lineEdit_centerY,
                    self.lineEdit_rectWidth,
                    self.lineEdit_rectHeight]
        widgets += dispTypeButtons
        widgets += [self.checkBox_TransparentBackground,
        ...
        self.comboBox_DEMLayer.currentIndexChanged.connect(
            self.demLayerChanged)
        self.horizontalSlider_DEMSize.valueChanged.connect(
            self.resolutionSliderChanged)
    ...
    def setup(self, properties=None, layer=None, isPrimary=True):
        ...
        self.setLayoutsVisible([self.formLayout_DEMLayer,
                                self.verticalLayout_Advanced,
                                self.formLayout_Surroundings],
                               isPrimary)
        self.setWidgetsVisible([self.radioButton_Advanced], isPrimary)
        self.setWidgetsVisible([self.toolButton_PointTool], False)
        self.setEnabled(
            isPrimary or self.dialog.currentItem.data(
        ...
        if isPrimary:
            ...
            self.connect(
                self.dialog.mapTool,
                SIGNAL("rectangleCreated()"),
                self.rectangleSelected)
            self.dialog.startPointSelection()
        else:
            # no sides with additional dem
            self.checkBox_Sides.setChecked(False)
    ...
    def initTextureSizeComboBox(self):
        ...
        for i in [4, 2, 1]:
            percent = i * 100
            text = "{0} %  ({1} x {2} px)".format(
                percent, outsize.width() * i, outsize.height() * i)
    ...
    def resolutionSliderChanged(self, v):
        ...
        QToolTip.showText(
            self.horizontalSlider_DEMSize.mapToGlobal(
                QPoint(
                    0,
                    0)),
            "about {0} x {0}".format(size),
    ...
    def browseClicked(self):
        ...
        filename = QFileDialog.getOpenFileName(
            self, "Select image file", directory, filterString)
        if filename:
    ...
    def surroundingsToggled(self, checked):
        ...
        self.setWidgetsEnabled([self.radioButton_ImageFile,
                                self.groupBox_Clip,
                                self.groupBox_Accessories],
                               is_simple_wo_surroundings)
    ...
    def updateDEMSize(self, v=None):
        ...
        self.label_Resolution.setText(
            "{0} x {1}".format(
                demSize.width(),
                demSize.height()))  # TODO: label_GridSize
    ...
    def dispTypeChanged(self, checked=True):
        if checked:
            ...
            self.setWidgetsEnabled(
                [self.label_TextureSize, self.comboBox_TextureSize], t in [0, 1])
    ...
    def samplingModeChanged(self, checked):
        ...
        self.setLayoutsEnabled(
            [self.gridLayout_Surroundings], isSimpleMode and surroundings)
        self.setWidgetsEnabled([self.radioButton_ImageFile,
                                self.groupBox_Clip,
                                self.groupBox_Accessories],
                               is_simple_wo_surroundings)
        ...
        if self.isPrimary:
            self.setLayoutsVisible(
                [self.horizontalLayout_Advanced1, self.horizontalLayout_Advanced3], isAdvancedMode)
            ...
            if isSimpleMode:
                ...
                try:
                    isPoint = (
                        float(
                            self.lineEdit_rectWidth.text()) == 0 and float(
                            self.lineEdit_rectHeight.text()) == 0)
                except ValueError:
                    isPoint = True
...
class VectorPropertyPage(PropertyPage, Ui_VectorPropertiesWidget):
    ...
    def __init__(self, dialog, parent=None):
        ...
        widgets = [
            ...
            self.heightWidget] + self.styleWidgets
        widgets += [self.radioButton_AllFeatures,
                    self.radioButton_IntersectingFeatures, self.checkBox_Clip]
        widgets += [self.checkBox_ExportAttrs,
                    self.comboBox_Label, self.labelHeightWidget]
        self.registerPropertyWidgets(widgets)
    ...
    def setup(self, properties=None, layer=None):
        ...
        if layer.geometryType() != QGis.Line:
            ...
            self.labelHeightWidget.setup(
                options={
                    "layer": layer,
                    "defaultValue": defaultLabelHeight /
                    mapTo3d.multiplierZ})
        else:
    ...
    def setupStyleWidgets(self, index=None):
        ...
        # setup widgets
        self.dialog.objectTypeManager.setupWidgets(
            self, mapTo3d, self.layer, self.layer.geometryType(), index)
    ...
    def altitudeModeChanged(self, index=None):
        ...
        if (geom_type == QGis.Line and type_index == 4) or (geom_type ==
                                                            QGis.Polygon and type_index == 1):    # Profile or Overlay
