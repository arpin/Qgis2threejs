...
class DEMBlock:
    ...
    def __init__(self, dem_width, dem_height, dem_values,
                 plane_width, plane_height, offsetX, offsetY):
    ...
    def write(self, writer):
        ...
        writer.write(
            "bl = lyr.addBlock({0}, {1});\n".format(
                pyobj2js(
                    self.properties), pyobj2js(
                    bool(
                        self.clip_geometry))))
        writer.write("bl.data = [{0}];\n".format(
            ",".join(map(gdal2threejs.formatValue, self.dem_values))))
        ...
        if self.clip_geometry:
            ...
            geom = PolygonGeometry.fromQgsGeometry(
                self.clip_geometry, z_func, transform_func)
            geom.splitPolygon(
                writer.triangleMesh(
                    self.dem_width,
                    self.dem_height))
            ...
            for polygon in geom.split_polygons:
                ...
                if len(polygon) == 1 and len(boundary) == 4:
                    ...
                    bnds = [map(lambda pt: [pt.x, pt.y], bnd)
                            for bnd in polygon]
                    polygons.append(bnds)
...
class DEMBlocks:
    ...
    def processEdges(self):
        ...
        for istop, neighbor in enumerate(
                [blocks[ci - size], blocks[ci + size]]):
    ...
    def stats(self):
        ...
        block = self.blocks[0]
        stats = {
            "max": block.orig_stats["max"],
            "min": block.orig_stats["min"]}
