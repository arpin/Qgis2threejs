...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent):
        ...
        # load settings
        settings = QSettings()
        ui.lineEdit_BrowserPath.setText(settings.value(
            "/Qgis2threejs/browser", "", type=unicode))
        enabled_plugins = QSettings().value(
            "/Qgis2threejs/plugins", "", type=unicode).split(",")
        ...
        for i, name in enumerate(plugins):
            ...
            item.setCheckState(
                Qt.Checked if name in enabled_plugins else Qt.Unchecked)
    ...
    def accept(self):
        ...
        # general settings
        settings.setValue(
            "/Qgis2threejs/browser",
            self.ui.lineEdit_BrowserPath.text())
