...
Q3D.ClippedDEMBlock.prototype = {
    ...
    buildSides: function(layer, material, z0) {
        ...
        layer.materials.push({
            type: Q3D.MaterialType.MeshLambert,
            m: mat_back
        });
...
Q3D.DEMLayer.prototype.buildFrame = function(block, color, z0) {
    ...
    this.materials.push({
        type: Q3D.MaterialType.LineBasic,
        m: mat
    });

    // horizontal rectangle at bottom
    var hw = dem.plane.width / 2,
        hh = dem.plane.height / 2;
    ...
    var pts = [
        [-hw, -hh, dem.data[dem.data.length - dem.width]],
...
Q3D.VectorLayer.prototype.buildLabels = function(parent, parentElement, getPointsFunc, zFunc) {
    ...
    var zShift = this.project.zShift,
        zScale = this.project.zScale;
    ...
    var line_mat = new THREE.LineBasicMaterial({
        color: Q3D.Options.label.connectorColor
    });
...
Q3D.PointLayer.prototype.build = function(parent) {
    ...
    if (this.objType == "Sphere") createGeometry = function(f) {
        ...
        createGeometry = function(f) {
            var geom = new THREE.CylinderGeometry(f.r, f.r, 0, 32),
                m = new THREE.Matrix4();
...
Q3D.Utils.putStick = function(x, y, zFunc, h) {
    if (Q3D.Utils._stick_mat === undefined) Q3D.Utils._stick_mat = new THREE.LineBasicMaterial({
        color: 0xff0000
    });
