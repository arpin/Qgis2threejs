...
class ObjectTreeItem:
    ...
    topItemNames = [
        ...
        "Polygon"]
    geomType2id = {
        QGis.Point: ITEM_POINT,
        QGis.Line: ITEM_LINE,
        QGis.Polygon: ITEM_POLYGON}
    ...
    def parentIdByLayer(cls, layer):
        ...
        if layerType == QgsMapLayer.RasterLayer and layer.providerType(
...
class MapTo3D:
    ...
    def __init__(self, mapSettings, planeWidth=100,
                 verticalExaggeration=1, verticalShift=0):
        # map canvas
...
class GDALDEMProvider(Raster):
    ...
    def _read(self, width, height, geotransform):
        ...
        fs = "f" * width * height
        return struct.unpack(fs, band.ReadRaster(
            0, 0, width, height, buf_type=gdal.GDT_Float32))
...
def createQuadTree(extent, p):
    ...
    """
    try:
        cx, cy, w, h = map(float, [p["lineEdit_centerX"], p["lineEdit_centerY"], p[
                           "lineEdit_rectWidth"], p["lineEdit_rectHeight"]])
    except ValueError:
        return None
    ...
    if not quadtree.buildTreeByRect(QgsRectangle(
            c.x() - hw, c.y() - hh, c.x() + hw, c.y() + hh), p["spinBox_Height"]):
        return None
