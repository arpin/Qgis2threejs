...
class Raster:
    ...
    def open(self, filename):
        filename_utf8 = filename.encode(
            "UTF-8") if isinstance(filename, unicode) else filename
...
def gdal2threejs(demfile, texfile, outfile="data.js",
                 title="no title", suffix=""):
    ...
    with open(outfile, "w") as f:
        ...
        plane = "{width:%f,height:%f,offsetX:0,offsetY:0}" % (
            100, 100 * extent_height / extent_width)
...
def formatValue(val, fmt="%.6f"):
    try:
        # to filter invalid value (e.g. nan, inf. supposed type of val is
        # float)
        int(val)
