...
class VectorPropertyReader:
    ...
    def __init__(self, objectTypeManager, layer, properties=None):
        ...
        if properties:
            ...
            typeitem = objectTypeManager.objectTypeItem(
                layer.geometryType(), self.item_index)  #
    ...
    def _readColor(self, widgetValues, f=None, isBorder=False):
        ...
        if symbol is None:
            logMessage(
                u'Symbol for feature cannot be found: {0}'.format(
                    self.layer.name()))
    ...
    def useZ(self):
        return self.properties["heightWidget"][
            "comboData"] == HeightWidgetFunc.Z_VALUE
    ...
    def relativeHeight(self, f=None):
        ...
        if vals["comboData"] in [HeightWidgetFunc.RELATIVE,
                                 HeightWidgetFunc.ABSOLUTE, HeightWidgetFunc.Z_VALUE] or f is None:
            return self.toFloat(vals["editText"])

        # attribute value + addend
        fieldName = vals["comboText"].lstrip("+").strip(' "')
        return self.toFloat(f.attribute(fieldName)) + \
            self.toFloat(vals["editText"])

        # if lst[0] >= HeightWidgetFunc.FIRST_ATTR_REL:
        #  return float(f.attributes()[lst[0] - HeightWidgetFunc.FIRST_ATTR_REL]) + float(lst[2])
        # return float(f.attributes()[lst[0] -
        # HeightWidgetFunc.FIRST_ATTR_ABS]) + float(lst[2])
            ...
            if widgetType in [StyleWidget.COLOR, StyleWidget.OPTIONAL_COLOR]:
                vals.append(
                    self._readColor(
                        widgetValues,
                        f,
                        widgetType == StyleWidget.OPTIONAL_COLOR))
                ...
                if comboData == FilePathWidgetFunc.FILEPATH or f is None:
                    ...
                    if value == NULL:
                        value = ""
                        logMessage(
                            u"Empty attribute value in the field '{0}'".format(fieldName))
                    vals.append(
                        os.path.join(
                            widgetValues["editText"],
                            value.strip('"')))
                ...
                if widgetValues["comboData"] in [HeightWidgetFunc.RELATIVE,
                                                 HeightWidgetFunc.ABSOLUTE, HeightWidgetFunc.Z_VALUE] or f is None:
                    vals.append(self.toFloat(widgetValues["editText"]))
                else:
                    # attribute value + addend
                    fieldName = widgetValues[
                        "comboText"].lstrip("+").strip(' "')
                    vals.append(
                        self.toFloat(
                ...
                if comboData == FieldValueWidgetFunc.ABSOLUTE or f is None:
                    ...
                    val = self.toFloat(f.attribute(fieldName)) * \
                        self.toFloat(widgetValues["editText"])
                    vals.append(str(val))
        return vals
