...
class ExportSettings:
    ...
    def controls(self):
        ...
        return QSettings().value("/Qgis2threejs/lastControls",
                                 def_vals.controls, type=unicode)
    ...
    def setMapCanvas(self, canvas):
        self.setMapSettings(
            canvas.mapSettings() if QGis.QGIS_VERSION_INT >= 20300 else canvas.mapRenderer())
        self.canvas = canvas
    ...
    def demProvider(self):
        layerId = self.data.get(
            ObjectTreeItem.ITEM_DEM, {}).get(
            "comboBox_DEMLayer", 0)
    ...
    def mapTo3d(self):
        ...
        verticalExaggeration = world.get(
            "lineEdit_zFactor", def_vals.zExaggeration)
        verticalShift = world.get("lineEdit_zShift", def_vals.zShift)
        self._mapTo3d = MapTo3D(
            self.mapSettings,
            float(baseSize),
    ...
    def demProviderByLayerId(self, id):
        ...
        if id.startswith("plugin:"):
            ...
            return GDALDEMProvider(layer.source(), str(self.crs.toWkt()), source_wkt=str(
