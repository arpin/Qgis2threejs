    ...
    def subdivideRecursively(self, rect, maxHeight):
        ...
        for y in range(2):
            for x in range(2):
                ...
                node = self.__class__(
                    self, quadrect, 2 * y + x, self.height + 1)
...
class QuadTree:
    ...
    def buildTreeByPoint(self, point, height):
        return self.buildTreeByRect(QgsRectangle(
            point.x(), point.y(), point.x(), point.y()), height)
    ...
    def neighbors(self, quad):
        # if neighbor count of one direction is not only one, returns one of
        # neighbors. so totally returns 4 neighbors.
        quads = [None] * 4
        ...
        quads[
            ...
            QgsPoint(
                ...
                rect.height()))
        quads[
            self.LEFT] = self.quadByPosition(
            QgsPoint(
                rect.xMinimum() -
                m *
                rect.width(),
                center.y()))
        quads[self.RIGHT] = self.quadByPosition(
            QgsPoint(rect.xMaximum() + m * rect.width(), center.y()))
        quads[
            self.DOWN] = self.quadByPosition(
            QgsPoint(
                center.x(),
                rect.yMinimum() -
                m *
        ...
        for row in range(height):
            ...
            for y in range(y0, self.dem_height):
                for col in range(width):
                    ...
                    dem_values += self.quads[col +
                                             row *
                                             width].dem_values[i:i +
                                                               self.dem_width -
                                                               x0]
        return dem_values
