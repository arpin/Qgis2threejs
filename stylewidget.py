...
class WidgetFuncBase:
    ...
    def setup(self, name, editLabel="Value", lineEdit="", placeholderText="",
              readOnly=False, toolButton=False, checkBox=False):
        # initialize widgets
...
class FieldValueWidgetFunc(WidgetFuncBase):
    ...
    def setup(self, options=None):
        ...
        self.widget.comboBox.addItem(
            "Fixed value", FieldValueWidgetFunc.ABSOLUTE)
...
class ColorWidgetFunc(WidgetFuncBase):
    ...
    def setup(self, options=None):
        ...
        WidgetFuncBase.setup(
            self,
            "Color",
            lineEdit=None,
            placeholderText="0xrrggbb")
        options = options or {}
    ...
    def toolButtonClicked(self):
        color = QColorDialog.getColor(
            QColor(
                self.widget.lineEdit.text().replace(
                    "0x", "#")))
        if color.isValid():
    ...
    def setValues(self, vals):
        ...
        if index != -1:
            self.widget.comboBox.setCurrentIndex(index)
            self.widget.comboBoxSelectionChanged(
                index)  # make sure to update visibility
        self.widget.lineEdit.setText(vals["editText"])
...
class FilePathWidgetFunc(WidgetFuncBase):
    ...
    def toolButtonClicked(self):
        ...
        if comboBox.itemData(comboBox.currentIndex()
                             ) == FilePathWidgetFunc.FILEPATH:
            filepath = QFileDialog.getOpenFileName(
                None, "Select a file", workdir, self.filterString)
            if filepath:
                self.widget.lineEdit.setText(filepath)
        else:
            directory = QFileDialog.getExistingDirectory(
                None, "Select a directory", workdir)
            if directory:
...
class HeightWidgetFunc(WidgetFuncBase):
    ...
    def setup(self, options=None):
        ...
        if layer and layer.wkbType() in [
                QGis.WKBPoint25D, QGis.WKBLineString25D, QGis.WKBMultiPoint25D, QGis.WKBMultiLineString25D]:
        ...
        if layer:
            ...
            for index, fieldName in index_fieldName:
                comboBox.addItem(
                    u'+"{0}"'.format(fieldName),
                    HeightWidgetFunc.FIRST_ATTR_REL + index)
                # note: VectorPropertyReader.relativeHeight() uses item name to
                # get field name
        ...
        if layer:
            for index, fieldName in index_fieldName:
                comboBox.addItem(
                    u' "{0}"'.format(fieldName),
                    HeightWidgetFunc.FIRST_ATTR_ABS + index)
                # note: VectorPropertyReader.relativeHeight() uses item name to
                # get field name
...
class LabelHeightWidgetFunc(WidgetFuncBase):
    ...
    def setup(self, options=None):
        ...
        if layer and layer.geometryType() != QGis.Point:
            # Will be initialized in obj_mod.setupWidgets() if polygon. Line
            # layer cannot have labels.
            return
        self.widget.comboBox.addItem(
            "Height from point",
            LabelHeightWidgetFunc.RELATIVE)
        self.widget.comboBox.addItem(
            "Fixed value", LabelHeightWidgetFunc.ABSOLUTE)

        if layer:
            self.widget.addFieldNames(layer)

    def comboBoxSelectionChanged(self, index):
        if self.widget.comboBox.itemData(
                index) < LabelHeightWidgetFunc.FIRST_ATTRIBUTE:
            label = "Value"
            defaultValue = self.defaultValue
        else:
            label = "Addend"
...
class TransparencyWidgetFunc(WidgetFuncBase):
    ...
    def setup(self, options=None):
        ...
        self.widget.comboBox.clear()
        self.widget.comboBox.addItem(
            "Feature style", TransparencyWidgetFunc.FEATURE)
        self.widget.comboBox.addItem(
            "Fixed value", TransparencyWidgetFunc.VALUE)
...
class OptionalColorWidgetFunc(ColorWidgetFunc):
    ...
    def setup(self, options=None):
        ...
        if itemText.get(OptionalColorWidgetFunc.NONE, "") is not None:
            self.widget.comboBox.insertItem(
                0, "None", OptionalColorWidgetFunc.NONE)
        ...
        index = self.widget.comboBox.findData(options.get(
    ...
    def addFieldNames(self, layer, fieldType=FIELDTYPE_NUMBER):
        ...
        for index, fieldName in index_fieldName:
            self.comboBox.addItem(
                u'"{0}"'.format(fieldName),
                WidgetFuncBase.FIRST_ATTRIBUTE + index)
            # note: VectorPropertyReader.values() uses item name to get field
            # name
