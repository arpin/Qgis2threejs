...
class RotatedRect:
    ...
    def clone(self):
        return RotatedRect(self._center, self._width,
    ...
    def subrectangle(self, norm_rect, y_inverted=False):
        ...
        return RotatedRect(rect.center(), rect.width(), rect.height()).rotate(
            self._rotation, self._center)

    @classmethod
    def fromMapSettings(cls, mapSettings):
        extent = mapSettings.visibleExtent(
        ) if QGis.QGIS_VERSION_INT >= 20300 else mapSettings.extent()
        ...
        return cls(extent.center(), mupp * canvas_size.width(),
                   mupp * canvas_size.height(), rotation)
    ...
    def geotransform(self, cols, rows, is_grid_point=True):
        ...
        if rotation:
            # rotate top-left corner of unrotated extent around center of
            # extent counter-clockwise (map rotates clockwise)
            rpt = self.rotatePoint(
                QgsPoint(
                    ur_rect.xMinimum(),
                    ur_rect.yMaximum()),
                rotation,
            ...
            c = math.cos(theta)
            s = math.sin(theta)
            geotransform = [rpt.x(), res_lr * c, res_ul * s,
                            rpt.y(), res_lr * s, -res_ul * c]
            if is_grid_point:
                # top-left corner of extent corresponds to center of top-left
                # pixel.
                geotransform[0] -= 0.5 * \
                    geotransform[1] + 0.5 * geotransform[2]
                geotransform[3] -= 0.5 * \
                    geotransform[4] + 0.5 * geotransform[5]
        else:
            xres = self._width / segments_x
            yres = self._height / segments_y
            geotransform = [
                ur_rect.xMinimum(), xres, 0, ur_rect.yMaximum(), 0, -yres]
            if is_grid_point:
    ...
    def vertices(self):
        ...
        if self._rotation:
            return map(lambda pt: self.rotatePoint(
                pt, self._rotation, self._center), pts)

        return pts

    def __repr__(self):
        return "RotatedRect(c:{0}, w:{1}, h:{2}, r:{3})".format(
            self._center.toString(), self._width, self._height, self._rotation)

        # print coordinates of vertices
        pts = self.verticies()
        return "RotatedRect:" + \
            ",".join(map(lambda x_y: "P{0}({1})".format(
                x_y[0], x_y[1].toString()), enumerate(pts)))
