...
class ImageManager(DataManager):
    ...
    def layerImageIndex(self, layerids, width, height,
                        extent, transp_background):
        img = (
            self.LAYER_IMAGE,
            (layerids,
             width,
    ...
    def mapCanvasImage(self, transp_background=False):
        ...
        if canvas is None or transp_background:
            ...
            ), self.exportSettings.baseExtent, transp_background)
    ...
    def renderedImage(self, width, height, extent,
                      transp_background=False, layerids=None):
        if QGis.QGIS_VERSION_INT < 20700:
            return self._renderedImage2(
                width, height, extent, transp_background, layerids)
        ...
        if has_pluginlayer:
            # use this method so that TileLayerPlugin layer is rendered
            # correctly
    ...
    def _renderedImage2(self, width, height, extent,
                        transp_background=False, layerids=None):
...
class MaterialManager(DataManager):
    ...
    def getMeshLambertIndex(self, color, transparency=0, doubleSide=False):
        return self._indexCol(self.MESH_LAMBERT, color,
                              transparency, doubleSide)

    def getSmoothMeshLambertIndex(
            self, color, transparency=0, doubleSide=False):
        return self._indexCol(self.MESH_LAMBERT_SMOOTH,
                              color, transparency, doubleSide)

    def getFlatMeshLambertIndex(self, color, transparency=0, doubleSide=False):
        return self._indexCol(self.MESH_LAMBERT_FLAT,
                              color, transparency, doubleSide)
    ...
    def getMapImageIndex(self, width, height, extent,
                         transparency=0, transp_background=False):
        mat = (
            self.MAP_IMAGE,
            (width,
             height,
    ...
    def getLayerImageIndex(self, layerids, width, height,
                           extent, transparency=0, transp_background=False):
        mat = (
            self.LAYER_IMAGE,
            (layerids,
             width,
    ...
    def getImageFileIndex(self, path, transparency=0,
                          transp_background=False, doubleSide=False):
        mat = (
            self.IMAGE_FILE,
            (path,
             transp_background),
    ...
    def write(self, f, imageManager):
        ...
        for index, mat in enumerate(self._list):
            ...
            if mat[0] == self.CANVAS_IMAGE:
                ...
                m["i"] = imageManager.mapImageIndex(
                    width, height, extent, transp_background)
            elif mat[0] == self.LAYER_IMAGE:
                layerids, width, height, extent, transp_background = mat[1]
                m["i"] = imageManager.layerImageIndex(
                    layerids, width, height, extent, transp_background)
...
class ModelManager(DataManager):
    ...
    def filesToCopy(self):
        ...
        if self._collada:
            f.append(
                {"files": ["js/threejs/loaders/ColladaLoader.js"], "dest": "threejs/loaders"})
        return f
            ...
            if exists and os.path.isfile(path):
                with open(path) as model_file:
                    data = model_file.read().replace(
                        ...
                        "\\n")
                f.write(
                    u"project.models[%d] = {type:'%s',data:'%s'};\n" %
                    (index, model_type, data))
            else:
                f.write(
                    u"project.models[%d] = {type:'%s',data:null};\n" %
                    (index, model_type))
