...
class PointGeometry:
    ...
    def fromQgsGeometry(geometry, z_func, transform_func):
        geom = PointGeometry()
        pts = geometry.asMultiPoint() if geometry.isMultipart() else [
            geometry.asPoint()]
        geom.pts = [
            transform_func(
                pt.x(),
    ...
    def fromOgrGeometry25D(geometry, transform_func):
        ...
        if geomType == ogr.wkbPoint25D:
            ...
            geoms = [
                geometry.GetGeometryRef(i) for i in range(
...
class LineGeometry:
    ...
    def asList(self):
        return [map(lambda pt: [pt.x, pt.y, pt.z], line)
    ...
    def fromQgsGeometry(geometry, z_func, transform_func):
        geom = LineGeometry()
        lines = geometry.asMultiPolyline() if geometry.isMultipart() else [
            geometry.asPolyline()]
        geom.lines = [[transform_func(pt.x(), pt.y(), z_func(
            pt.x(), pt.y())) for pt in line] for line in lines]
        return geom
        ...
        for polygon in polygons:
            if useCentroidHeight or calcCentroid:
                ...
                if calcCentroid and centroidPerPolygon:
                    geom.centroids.append(transform_func(
                        pt.x(), pt.y(), centroidHeight))
...
class TriangleMesh:
    ...
    def __init__(self, xmin, ymin, xmax, ymax, x_segments, y_segments):
        ...
        for x in range(x_segments):
            addVBand(x, QgsGeometry.fromRect(QgsRectangle(
                xmin + x * xres, ymin, xmin + (x + 1) * xres, ymax)))

        for y in range(y_segments):
            addHBand(y, QgsGeometry.fromRect(QgsRectangle(
                xmin, ymax - (y + 1) * yres, xmax, ymax - y * yres)))
            ...
            for y in self.hIntersects(vi):
                ...
                if geom.contains(quad):
                    ...
                    for i, tri in enumerate(
                            map(QgsGeometry.fromPolygon, tris)):
...
class Triangles:
    ...
    def __init__(self):
        self.vertices = []
        self.faces = []
        # dict to find whether a vertex already exists: [y][x] = vertex index
        self.vdict = {}
