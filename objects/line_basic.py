    ...
    if type_index in [1, 2]:  # Pipe, Cone
        ppage.addStyleWidget(
            StyleWidget.FIELD_VALUE, {
                "name": "Radius", "defaultValue": defaultValue, "layer": layer})
    elif type_index == 3:     # Box
        ppage.addStyleWidget(
            StyleWidget.FIELD_VALUE, {
                "name": "Width", "defaultValue": defaultValue, "layer": layer})
        ppage.addStyleWidget(
            StyleWidget.FIELD_VALUE, {
                "name": "Height", "defaultValue": defaultValue, "layer": layer})
...
def layerProperties(writer, layer):
    ...
    if prop.type_index == 4:      # Profile
        ...
        isBRelative = (cb == HeightWidgetFunc.RELATIVE or cb >=
                       HeightWidgetFunc.FIRST_ATTR_REL)
...
def write(writer, layer, feat):
    ...
    if type_index == 0:   # Line
        ...
        if rb != 0:
            ...
            writer.writeFeature(
                {"m": mat, "lines": feat.geom.asList(), "rt": rt, "rb": rb})
        ...
        writer.writeFeature(
            {"m": mat, "lines": feat.geom.asList(), "w": w, "h": h})

    elif type_index == 4:   # Profile
        d = {
            "m": layer.materialManager.getFlatMeshLambertIndex(
                vals[0],
                vals[1],
                doubleSide=True)}
